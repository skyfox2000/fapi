      ...
      fields.forEach((field) => {
        if (field.indexOf('.') > -1) {
          ...
          for (let i = 0; i < subKeys.length; i++) {
            ...
            if (typeof target[subKeys[i]] === 'object' && !Array.isArray(target[subKeys[i]])) {
      ...
      fields.forEach((field) => {
        if (field.indexOf('.') > -1) {
          ...
          for (let i = 0; i < subKeys.length; i++) {
            ...
            } else if (source[subKeys[i]] === null || source[subKeys[i]] === undefined) {
              break
            } else {
              if (target[subKeys[i]] === undefined) {
                if (typeof source[subKeys[i]] !== 'object' || Array.isArray(source[subKeys[i]])) {
                  target[subKeys[i]] = source[subKeys[i]]
                  break
