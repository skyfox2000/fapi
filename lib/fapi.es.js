...
let I = null, V = null;
const v = (e, t) => {
  We() && (t !== void 0 ? t instanceof Uint8Array ? console.log(`[Crypto Debug] ${e}:`, $(t)) : typeof t == "object" ? console.log(`[Crypto Debug] ${e}:`, JSON.stringify(t, null, 2)) : console.log(`[Crypto Debug] ${e}:`, t) : console.log(`[Crypto Debug] ${e}`));
}, Ne = (e) => I !== null ? !1 : (I = e, !0), je = () => I !== null, Fe = async (e) => {
  ...
  try {
    ...
    V = t;
    const r = await Be(t);
    v("AES 密钥 (Base64)", $(r)), v("AES 密钥 (Hex)", Array.from(r).map((a) => a.toString(16).padStart(2, "0")).join(""));
    const o = await ze(r, I);
    v("RSA 加密后的 AES 密钥 (X-Encrypted-Key)", o);
    const { ciphertext: s, nonce: u } = await Xe(e, t);
    ...
    return console.error("加密请求数据失败:", t), V = null, null;
  }
}, Ve = async (e, t) => {
  if (!V)
  ...
  try {
    ...
    const o = await Ge(e, t, V);
    return v("解密后的数据", o), v("========== 解密响应结束 =========="), V = null, o;
  } catch (r) {
    return console.error("解密响应数据失败:", r), v("解密错误", r), V = null, null;
  }
}, me = (e) => typeof e == "string" && e.length > 0 && /^[A-Za-z0-9+/=]+$/.test(e);
let U = {
  enabled: !1,
  includeApis: [],
  excludeApis: [],
...
const kt = (e) => {
...
}, We = () => U.debug === !0, Qe = () => je(), Ye = (e, t) => {
  if (!Qe() || t && U.excludeHostKeys && U.excludeHostKeys.length > 0 && U.excludeHostKeys.includes(t) || U.includeHostKeys && U.includeHostKeys.length > 0 && (!t || !U.includeHostKeys.includes(t)))
    return !1;
  if (U.excludeApis && U.excludeApis.length > 0) {
    for (const r of U.excludeApis)
      if (typeof r == "string") {
        if (e.includes(r))
          return !1;
  ...
  if (!U.includeApis || U.includeApis.length === 0)
    return !0;
  for (const r of U.includeApis)
    if (typeof r == "string") {
      if (e.includes(r))
        return !0;
    } else if (r instanceof RegExp && r.test(e))
...
const Ot = (e) => {
  ...
  t && (Le = t), r && (He = r), Object.assign(xe, o);
}, Ze = () => xe, Ie = () => Le, te = () => He, se = (e) => {
  ...
  const c = (w, S, A) => w & S | ~w & A, g = (w, S, A) => w & A | S & ~A, p = (w, S, A) => w ^ S ^ A, T = (w, S, A) => S ^ (w | ~A), P = [
    3614090360,
    3905402710,
    606105819,
    3250441966,
  ...
  for (; N < t.length; ) {
    ...
    for (let h = 0; h < 64; h++) {
      let l, b, E;
      h < 16 ? (l = c(A, O, x), b = h) : h < 32 ? (l = g(A, O, x), b = (5 * h + 1) % 16) : h < 48 ? (l = p(A, O, x), b = (3 * h + 5) % 16) : (l = T(A, O, x), b = 7 * h % 16), E = x, x = O, O = A, A = A + $e(S + l + P[h] + w[b] | 0, j[h]), S = E;
    }
    o = o + S | 0, s = s + A | 0, u = u + O | 0, a = a + x | 0, N += 16;
  }
  return [o, s, u, a].map((w) => {
    const S = w & 255, A = w >>> 8 & 255, O = w >>> 16 & 255;
    return [w >>> 24 & 255, O, A, S];
  }).flat().map((w) => w.toString(16).padStart(2, "0")).join("");
}, tt = (e) => et(e), ae = (e, t, r, o) => {
  if (!t) return e;
  let s = se(t);
  ...
  set({
    ...
    if (a == null) return;
    const g = ae(e, t, o, r), p = c !== -1 ? Date.now() + c : void 0, T = `frontCache::${g}`, P = {
      data: a,
      expireAt: p,
...
var we = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, Y = {}, F = {}, L = {}, m = {}, Se;
function st() {
  if (Se) return m;
  Se = 1, Object.defineProperty(m, "__esModule", { value: !0 }), m.toBig = m.shrSL = m.shrSH = m.rotrSL = m.rotrSH = m.rotrBL = m.rotrBH = m.rotr32L = m.rotr32H = m.rotlSL = m.rotlSH = m.rotlBL = m.rotlBH = m.add5L = m.add5H = m.add4L = m.add4H = m.add3L = m.add3H = void 0, m.add = A, m.fromBig = r, m.split = o;
  ...
  const j = (f, i) => f;
  m.rotr32L = j;
  const N = (f, i, d) => f << d | i >>> 32 - d;
  m.rotlSH = N;
  const k = (f, i, d) => i << d | f >>> 32 - d;
  m.rotlSL = k;
  const w = (f, i, d) => i << d - 32 | f >>> 64 - d;
  m.rotlBH = w;
  const S = (f, i, d) => f << d - 32 | i >>> 64 - d;
  m.rotlBL = S;
  function A(f, i, d, _) {
    const R = (i >>> 0) + (_ >>> 0);
  ...
  m.add3L = O;
  const x = (f, i, d, _) => i + d + _ + (f / 2 ** 32 | 0) | 0;
  m.add3H = x;
  const h = (f, i, d, _) => (f >>> 0) + (i >>> 0) + (d >>> 0) + (_ >>> 0);
  m.add4L = h;
  const l = (f, i, d, _, R) => i + d + _ + R + (f / 2 ** 32 | 0) | 0;
  m.add4H = l;
  const b = (f, i, d, _, R) => (f >>> 0) + (i >>> 0) + (d >>> 0) + (_ >>> 0) + (R >>> 0);
  m.add5L = b;
  const E = (f, i, d, _, R, K) => i + d + _ + R + K + (f / 2 ** 32 | 0) | 0;
  m.add5H = E;
  const B = {
    fromBig: r,
    split: o,
...
function at() {
  return Te || (Te = 1, (function(e) {
    ...
    const S = /* @ts-ignore */ typeof Uint8Array.from([]).toHex == "function" && typeof Uint8Array.fromHex == "function", A = /* @__PURE__ */ Array.from({ length: 256 }, (n, y) => y.toString(16).padStart(2, "0"));
    function O(n) {
      if (s(n), S)
        return n.toHex();
      let y = "";
    ...
    function l(n) {
      ...
      for (let D = 0, M = 0; D < H; D++, M += 2) {
        const pe = h(n.charCodeAt(M)), ge = h(n.charCodeAt(M + 1));
        if (pe === void 0 || ge === void 0) {
          const Ke = n[M] + n[M + 1];
          throw new Error('hex string expected, got non-hex character "' + Ke + '" at index ' + M);
        }
        C[D] = pe * 16 + ge;
      }
      return C;
    ...
    function _(...n) {
      ...
      for (let C = 0, D = 0; C < n.length; C++) {
        const M = n[C];
        H.set(M, D), D += M.length;
      }
      return H;
    }
    function R(n, y) {
    ...
    e.Hash = K;
    function W(n) {
      const y = (C) => n().update(i(C)).digest(), H = n();
      return y.outputLen = H.outputLen, y.blockLen = H.blockLen, y.create = () => n(), y;
    }
    function Q(n) {
      const y = (C, D) => n(D).update(i(C)).digest(), H = n({});
    ...
    e.wrapConstructor = W, e.wrapConstructorWithOpts = Q, e.wrapXOFConstructorWithOpts = J;
    function Pe(n = 32) {
...
function it() {
  if (Ee) return L;
  Ee = 1, Object.defineProperty(L, "__esModule", { value: !0 }), L.shake256 = L.shake128 = L.keccak_512 = L.keccak_384 = L.keccak_256 = L.keccak_224 = L.sha3_512 = L.sha3_384 = L.sha3_256 = L.sha3_224 = L.Keccak = void 0, L.keccakP = S;
  const e = /* @__PURE__ */ st(), t = /* @__PURE__ */ at(), r = BigInt(0), o = BigInt(1), s = BigInt(2), u = BigInt(7), a = BigInt(256), c = BigInt(113), g = [], p = [], T = [];
  ...
  const P = (0, e.split)(T, !0), j = P[0], N = P[1], k = (h, l, b) => b > 32 ? (0, e.rotlBH)(h, l, b) : (0, e.rotlSH)(h, l, b), w = (h, l, b) => b > 32 ? (0, e.rotlBL)(h, l, b) : (0, e.rotlSL)(h, l, b);
  function S(h, l = 24) {
    ...
    for (let E = 24 - l; E < 24; E++) {
      ...
      for (let i = 0; i < 10; i += 2) {
        const d = (i + 8) % 10, _ = (i + 2) % 10, R = b[_], K = b[_ + 1], W = k(R, K, 1) ^ b[d], Q = w(R, K, 1) ^ b[d + 1];
        for (let J = 0; J < 50; J += 10)
          h[i + J] ^= W, h[i + J + 1] ^= Q;
      }
      let B = h[2], f = h[3];
      for (let i = 0; i < 24; i++) {
        const d = p[i], _ = k(B, f, d), R = w(B, f, d), K = g[i];
  ...
  class A extends t.Hash {
    ...
    writeInto(l) {
      ...
      for (let B = 0, f = l.length; B < f; ) {
        ...
        const i = Math.min(E - this.posOut, f - B);
    ...
    _cloneInto(l) {
      const { blockLen: b, suffix: E, outputLen: B, rounds: f, enableXOF: i } = this;
      return l || (l = new A(b, E, B, i, f)), l.state32.set(this.state32), l.pos = this.pos, l.posOut = this.posOut, l.finished = this.finished, l.rounds = f, l.suffix = E, l.outputLen = B, l.enableXOF = i, l.destroyed = this.destroyed, l;
    }
  }
  L.Keccak = A;
  const O = (h, l, b) => (0, t.createHasher)(() => new A(l, h, b));
  ...
  const x = (h, l, b) => (0, t.createXOFer)((E = {}) => new A(l, h, E.dkLen === void 0 ? b : E.dkLen, !0));
...
function ct() {
  if (ke) return F;
  ke = 1;
  const { sha3_512: e } = /* @__PURE__ */ it(), t = 24, r = 32, o = (k = 4, w = Math.random) => {
    let S = "";
    for (; S.length < k; )
  ...
  const u = (k = "") => s(e(k)).toString(36).slice(1), a = Array.from(
    { length: 26 },
    (k, w) => String.fromCharCode(w + 97)
  ), c = (k) => a[Math.floor(k() * a.length)], g = ({
    globalObj: k = typeof we < "u" ? we : typeof window < "u" ? window : {},
    random: w = Math.random
  } = {}) => {
    const S = Object.keys(k).toString(), A = S.length ? S + o(r, w) : o(r, w);
    return u(A).substring(0, r);
  }, p = (k) => () => k++, T = 476782367, P = ({
    ...
    length: S = t,
    fingerprint: A = g({ random: k })
  } = {}) => function() {
    const x = c(k), h = Date.now().toString(36), l = w().toString(36), b = o(S, k), E = `${h + b + l + A}`;
    return `${x + u(E).substring(1, S)}`;
  }, j = P(), N = (k, { minLength: w = 2, maxLength: S = r } = {}) => {
    const A = k.length, O = /^[0-9a-z]+$/;
    try {
      if (typeof k == "string" && A >= w && A <= S && O.test(k))
        return !0;
    } finally {
    }
    return !1;
  };
  return F.getConstants = () => ({ defaultLength: t, bigLength: r }), F.init = P, F.createId = j, F.bufToBigInt = s, F.createCounter = p, F.createFingerprint = g, F.isCuid = N, F;
}
var Oe;
function ut() {
  if (Oe) return Y;
...
const ft = async (e, t) => {
  ...
  if (t && e.data)
    try {
      let s = null;
      if (typeof e.data == "string" && me(e.data) ? s = e.data : typeof e.data == "object" && e.data !== null && "encrypted" in e.data && typeof e.data.encrypted == "string" && (s = e.data.encrypted, console.log("[Crypto] 检测到 JSON 格式加密响应:", { encrypted: e.data.encrypted.slice(0, 50) + "..." })), s && me(s)) {
        const u = await Ve(s, t);
        if (u !== null)
          return u;
      }
    } catch (s) {
  ...
  t && (Ne(t) || console.warn("[Crypto] 公钥已存在，忽略响应头中的公钥（加密方式初始化后不可更改）"));
}, Ce = /\$\{([\w\.\[\]0-9]+)\}/g, yt = (e, t) => e.replace(Ce, (r, o) => {
  var a;
  const s = o.split(".");
  let u = t;
  for (const c of s) {
  ...
  return o.forEach((s) => {
    if (s && typeof s == "object") {
      ...
      s.children && Array.isArray(s.children) && (s.children = _e(e, s.children));
    }
  }), r ? t : o[0];
};
let Re = "";
const pt = () => Re, Lt = (e) => {
  ...
  if (ht(r.header), r.data = await dt(r), r.statusCode >= 200 && r.statusCode < 400) {
    ...
    if (s.status === z.SUCCESS) {
      const u = s.data;
      e.method === "POST" && t.fieldMap && u && _e(t.fieldMap, u);
      const a = te();
  ...
  if (await St(e, t), e.loadingText && q.loading({
  ...
  if (e.method === "POST") {
    ...
    const g = r(e, t).then((p) => {
      ...
      let T = ne[a];
      return T.result = p, T.expire = Date.now() + mt, ne[a] = T, p;
    }).finally(() => {
      t.loading = !1;
    });
    return ne[a] = {
  ...
  return new Promise((u) => {
    Me.request({
      ...
      var c;
      a.response && a.response.status && a.response.status > 200 && a.response.status < 600 ? await he(
        e,
        t,
        {
...
}, Ut = (e) => typeof e == "object" && e !== null && !Array.isArray(e) || Array.isArray(e);
export {
  rt as API_HOST,
  de as FrontCache,
  xt as ICON_HOST,
