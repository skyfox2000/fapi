...
const D = (e, t) => {
  ...
  if (!$)
    return null;
  try {
    D("========== 加密请求开始 =========="), D("原始数据", e), D("RSA 公钥", $);
    const t = await Qe(), r = await Ye(t);
    D("AES 密钥 (Base64)", oe(r)), D("AES 密钥 (Hex)", Array.from(r).map((c) => c.toString(16).padStart(2, "0")).join(""));
    const n = await Ze(r, $);
    D("RSA 加密后的 AES 密钥 (X-Encrypted-Key)", n);
    const { ciphertext: o, nonce: a } = await Ie(e, t);
    return D("AES 加密后的数据 (encryptedData)", o), D("Nonce (X-Nonce)", a), D("========== 加密请求结束 =========="), {
      encryptedData: o,
      encryptedKey: n,
      nonce: a,
      aesKey: t
  ...
  D("========== 解密响应开始 =========="), D("加密数据 (ciphertext)", e), D("响应 Nonce (X-Response-Nonce)", t);
  try {
    const n = await $e(e, t, r);
    return D("解密后的数据", n), D("========== 解密响应结束 =========="), n;
  } catch (n) {
    return console.error("解密响应数据失败:", n), D("解密错误", n), D("========== 解密响应结束 =========="), null;
  }
...
const _t = (e) => {
  ...
  if (!rt() || r && U.excludeSubApps && U.excludeSubApps.length > 0 && U.excludeSubApps.includes(r) || t && U.excludeHostKeys && U.excludeHostKeys.length > 0 && U.excludeHostKeys.includes(t) || U.includeHostKeys && U.includeHostKeys.length > 0 && (!t || !U.includeHostKeys.includes(t)))
    return !1;
  if (U.excludeApis && U.excludeApis.length > 0) {
    for (const n of U.excludeApis)
      if (typeof n == "string") {
        if (e.includes(n))
          return !1;
...
const gt = async (e, t) => {
  var o;
  const r = { encrypted: !1 };
  if (!nt(e.url || "", t.api, t.subApp) || ((o = e.method) == null ? void 0 : o.toUpperCase()) === "GET" || !e.data)
    return r;
  try {
